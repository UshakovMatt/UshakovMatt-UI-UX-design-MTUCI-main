...
const Details: FC = () => {
  ...
  return (
    ...
    <div
      className={`py-10 ${currentTheme == "black" ? "Ob1127" : "#2E0854"} bg`}>
      ...
      {!loading ? (
        <section className="min-h-screen flex items-center flex-col pb-20 px-4 relative">
            ...
            <div>
              ...
              <div className="flex justify-between mb-8">
                <button
                  ...
                  className="py-6 px-6 lex items-center justify-center border text-white bg-purple-300 font-extrabold cursor-pointer rounded-md hover:bg-rose-600 z-index-5">
                  Watch now
                </button>
                <button
                  onClick={() => {
                  ...
                  className="py-4 px-6 flex-items-center justify-center border bg-purple-300 font-extrabold cursor-pointer text-white rounded-md hover:bg-rose-600 z-index-5">
                  Download
                </button>
              </div>
            </div>
                      ...
                      <div className="text-white mb-3 text-xl flex flex-wrap items-baseline">
                          ...
                          Genres:</p>
                        {
                          movieDetails?.genres?.map((genre, index) => (
                            <b
                              className="bg-purple-200 p-2 text-base rounded-md ml-2 mb-3"
                              key={index}>
                              {genre}
                            </b>
                          ))
                  ...
                  {showDownload &&
                    movieDetails?.torrents?.map((torrent, index) => (
                      <a
                        href={torrent.url}
                        key={index}
                        className="flex border-2 bg-purple-300 text-black p-3 mb-2 rounded-md items-center border-transparent hover:bg-rose-500 w-56 h-14 mr-5">
                        <Image
                          src={download}
                          alt="Download"
          ...
          <div className="container flex flex-col">
            <p className="text-3xl text-black mb-5">Comments</p>
            ...
            <input
              ...
              value={comment.name}
              type="text"
              className="w-80 bg-transparent border-2 p-2 mb-2 rounded-md text-black"
            />
            <p className="text-black mb-2">Comment</p>
            <textarea
              value={comment.text}
              onChange={onCommentChange}
              name="text"
              className="w-full bg-transparent border-2 p-2 rounded-md text-black"
            />
            <button
              onClick={onSendComment}
              className="ml-auto text-black border-2 rounded-md p-3 mt-3 hover:bg-black">
              Send
            </button>
              ...
              {comments.length != 0 ?
                comments?.map((comment, index) => (
                  <div
                    ...
                    <p className="text-white-500">{comment.text}</p>
                    <button
                ...
                <div className="flex flex-col border bg-black w-full p-5 text-white rounded-md mb-3 text-2xl">
                  <span>
