...
export const Card: FC<CardProps> = ({
  ...
  return (
    <div
      ...
      {mouseOver && (
        <div
          className="absolute flex items-center justify-center flex-col text-center  bg-blue-200 p-5"
          style={{ width: "317px", height: "477px" }}>
          <h1 className="text-white text-3xl text-extrabold mb-3">{title}</h1>
          <p className="text-lg">Rating: {rating}</p>
          <button
            onClick={onFilmClick}
            className="p-3 border-2 mt-5 border-black px-10 rounded-md border-grey hover:bg-white ">
            Details
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col p-5 max-w-xs h-full">
        <span className="text-extrabold text-xl text-black title">
          {title} {year}
        </span>
